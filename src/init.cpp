...
void Shutdown()
{
    ...
    RenameThread("TransFast-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: TransFast.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: TransFastd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeTransFastamount=<n> " + _("Keep N TransFast anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. TransFast is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. TransFast is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("TransFast version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "TransFast server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of TransFast") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart TransFast to complete") << "\n";
    ...
    nAnonymizeTransFastAmount = GetArg("-anonymizeTransFastamount", 0);
    if(nAnonymizeTransFastAmount > 999999) nAnonymizeTransFastAmount = 999999;
    if(nAnonymizeTransFastAmount < 2) nAnonymizeTransFastAmount = 2;
    ...
    LogPrintf("Anonymize TransFast Amount %d\n", nAnonymizeTransFastAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1FASTX+1000 == (.1FASTX+100)*10
       10FASTX+10000 == (1FASTX+1000)*10
    */
