...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("TransFast"))
        return false;
...
{
    // Convert TransFast:// to TransFast:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("TransFast://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "TransFast:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "TransFast.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for TransFast.lnk
...
{
    return GetAutostartDir() / "TransFast.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a TransFast.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=TransFast\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the TransFast app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add TransFast app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("TransFast-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  TransFast-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("TransFast-Qt"));
