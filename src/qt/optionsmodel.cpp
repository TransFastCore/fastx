...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeTransFastAmount"))
        settings.setValue("nAnonymizeTransFastAmount", 1000);
    nAnonymizeTransFastAmount = settings.value("nAnonymizeTransFastAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeTransFastAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeTransFastAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeTransFastAmount:
            return QVariant(nAnonymizeTransFastAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeTransFastAmount:
            nAnonymizeTransFastAmount = value.toInt();
            settings.setValue("nAnonymizeTransFastAmount", nAnonymizeTransFastAmount);
            emit AnonymizeTransFastAmountChanged(nAnonymizeTransFastAmount);
            break;
        default:
            break;
        }
