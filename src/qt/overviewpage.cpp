...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeTransFastAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeTransFastAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeTransFastAmount = strAnonymizeTransFastAmount.remove(strAnonymizeTransFastAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTransFastAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeTransFastAmount*COIN) nMaxToAnonymize = nAnonymizeTransFastAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeTransFastAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeTransFastAmount));
        strAnonymizeTransFastAmount = strAnonymizeTransFastAmount.remove(strAnonymizeTransFastAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeTransFastAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeTransFastAmount == 0){
