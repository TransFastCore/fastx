...
int nAnonymizeTransFastAmount = 1000;
...
#else
    const char* pszModule = "TransFast";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\TransFast
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\TransFast
    // Mac: ~/Library/Application Support/TransFast
    // Unix: ~/.TransFast
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "TransFast";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "TransFast.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "TransFastd.pid"));
