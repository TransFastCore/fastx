...
const string strMessageMagic = "TransFast Signed Message:\n";
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
	...
	else if (pindexBest->nHeight < 50001) {
		nSubsidy = 1000 * COIN;
	}
	...
	else if (pindexBest->nHeight < 150001) {
		nSubsidy = 5 * COIN;
	}
	else if (pindexBest->nHeight < 200001) {
		nSubsidy = 5 * COIN;
	}
	else if (pindexBest->nHeight < 250001) {
		nSubsidy = 5 * COIN;
	}
	else {
		nSubsidy = 8 * COIN;
		...
		if (wallet.CreateCoinStake(wallet, nBits, nSearchInterval, nFees, txCoinStake, key))
		{
			if (txCoinStake.nTime >= pindexBest->GetPastTimeLimit() + 1)
			{
				...
				for (vector<CTransaction>::iterator it = vtx.begin(); it != vtx.end();)
					if (it->nTime > nTime) { it = vtx.erase(it); }
					else { ++it; }
...
{
	RenameThread("TransFast-loadblk");
